...
V1_4 Fault Log Added, Bypass Log Added, Implemented Threading For Keeping Omron/MySQL Connection Active & Maintain A Clean Exit
'''
...
class GuiPart(customtkinter.CTk):
    ...
    def __init__(self, master, queue_, endCommand):
        ...
        # Left Side Bar --> Bypass Log
        self.button_mainMenu_bypassLog = customtkinter.CTkButton(master=self.frame, text="Bypass Log", command=self.open_bypassLog)
        self.button_mainMenu_bypassLog.place(x=112, y=300, anchor='center')

        # Left Side Bar --> Setup
        title = customtkinter.CTkLabel(master=self.frame, text="Setup:", font=customtkinter.CTkFont(size=14))
        title.place(x=112, y=350, anchor='center')
        ...
        self.button_mainMenu_tagSetup.place(x=112, y=400, anchor='center')
        ...
        self.button_mainMenu_tagImport.place(x=112, y=450, anchor='center')
        ...
        self.button_mainMenu_configuration.place(x=112, y=500, anchor='center')
        ...
        self.button_mainMenu_bypassSetup.place(x=112, y=550, anchor='center')
        ...
        self.button_mainMenu_faultLogSetup.place(x=112, y=600, anchor='center')
        ...
        self.ui_thread2 = threading.Thread(target=self.databaseAlive, args=(self.event,))
        self.ui_thread2.start()
    ...
    def open_main(self):
        ...
        self.updateDatabaseList() # what function is this1???
    ...
    def open_faultLog(self):
        ...
        # Update Fault Log
        self.faultLog = self.updateFaultLog()
        ...
        # Setup Scrollable Frame Window
        self.tree_frameFaultLog = customtkinter.CTkFrame(master=self.frame_faultLog, border_color="#EB8613",
                                                         border_width=2)
        self.tree_frameFaultLog.place(x=self.frameSizeW / 2, y=40, anchor='n', height=950, width=1404)

        tree_scrollY = customtkinter.CTkScrollbar(self.tree_frameFaultLog, width=15)
        tree_scrollY.pack(side='right', fill='y', padx=4, pady=2)

        # Setup Tree View
        self.my_treeFaultLog = ttk.Treeview(master=self.tree_frameFaultLog, height=45, yscrollcommand=tree_scrollY.set)

        # Configure the scrollbar
        tree_scrollY.configure(command=self.my_treeFaultLog.yview)

        # Need to get the column list to populate columns correctly. This column list drives the table creation

        # Define Our Columns
        self.my_treeFaultLog['columns'] = ("Time","Fault")

        # Format Our Columns / Create Headings
        self.my_treeFaultLog.column("#0", width=0, minwidth=0, stretch='NO')
        self.my_treeFaultLog.heading("#0", text="", anchor='w')
        for column in self.my_treeFaultLog['columns']:
            if column == "Fault":
                self.my_treeFaultLog.column(column, anchor='w', width=1150, minwidth=25)
            else:
                self.my_treeFaultLog.column(column, anchor='w', width=200, minwidth=25)
            self.my_treeFaultLog.heading(column, text=column, anchor='w')

        # Pack The Tree Widget
        self.my_treeFaultLog.pack(padx=(8, 0), pady=(8, 2))

        # Treeview Data
        data = self.faultLog

        # Configure Treeview Colour Scheme
        self.my_treeFaultLog.tag_configure('oddrow', background="lightgray", foreground='#1F538D')
        self.my_treeFaultLog.tag_configure('evenrow', background="#1F538D", foreground='white')
        ...
        for record in data:
            # Alternating Line Colour
            if count % 2 == 0:  # If we divide count by 2 and the remainder is 0 (even Row)
                self.my_treeFaultLog.insert(parent='', index='end', iid=count, text="", values=record,
                                            tags=('evenrow',))
            else:
                self.my_treeFaultLog.insert(parent='', index='end', iid=count, text="", values=record,
                                            tags=('oddrow',))
    ...
    def open_bypassLog(self):
        self.closeWindows()
        self.activeWindow = 5
        self.setMainMenuColors()
        self.connection.eventLogInsert(datetime.now(), 3, "Bypass Log Screen Opened")

        # Update Bypass Log
        self.bypassLog = self.updateBypassLog()

        # Configure Tag Overview Viewing Frame
        self.frame_bypassLog = customtkinter.CTkFrame(master=self.master, width=self.frameSizeW, height=self.frameSizeH, corner_radius=0, fg_color="transparent")
        self.frame_bypassLog.place(x=self.framePosX, y=self.framePosY)

        # Tag Import Viewing Frame --> Title
        title = customtkinter.CTkLabel(master=self.frame_bypassLog, text="Bypass Log", font=customtkinter.CTkFont(size=20, weight="bold"))
        title.place(x=self.frameSizeW/2,y=20, anchor='center')

        # Setup Scrollable Frame Window
        self.tree_frameBypassLog = customtkinter.CTkFrame(master=self.frame_bypassLog, border_color="#EB8613",
                                                         border_width=2)
        self.tree_frameBypassLog.place(x=self.frameSizeW / 2, y=40, anchor='n', height=950, width=1404)

        tree_scrollY = customtkinter.CTkScrollbar(self.tree_frameBypassLog, width=15)
        tree_scrollY.pack(side='right', fill='y', padx=4, pady=2)

        # Setup Tree View
        self.my_treeBypassLog = ttk.Treeview(master=self.tree_frameBypassLog, height=45, yscrollcommand=tree_scrollY.set)

        # Configure the scrollbar
        tree_scrollY.configure(command=self.my_treeBypassLog.yview)

        # Need to get the column list to populate columns correctly. This column list drives the table creation

        # Define Our Columns
        self.my_treeBypassLog['columns'] = ("Time","Bypass")

        # Format Our Columns / Create Headings
        self.my_treeBypassLog.column("#0", width=0, minwidth=0, stretch='NO')
        self.my_treeBypassLog.heading("#0", text="", anchor='w')
        for column in self.my_treeBypassLog['columns']:
            if column == "Bypass":
                self.my_treeBypassLog.column(column, anchor='w', width=1150, minwidth=25)
            else:
                self.my_treeBypassLog.column(column, anchor='w', width=200, minwidth=25)
            self.my_treeBypassLog.heading(column, text=column, anchor='w')

        # Pack The Tree Widget
        self.my_treeBypassLog.pack(padx=(8, 0), pady=(8, 2))

        # Treeview Data
        data = self.bypassLog

        # Configure Treeview Colour Scheme
        self.my_treeBypassLog.tag_configure('oddrow', background="lightgray", foreground='#1F538D')
        self.my_treeBypassLog.tag_configure('evenrow', background="#1F538D", foreground='white')
        ...
        for record in data:
            # Alternating Line Colour
            if count % 2 == 0:  # If we divide count by 2 and the remainder is 0 (even Row)
                self.my_treeBypassLog.insert(parent='', index='end', iid=count, text="", values=record,
                                            tags=('evenrow',))
            else:
                self.my_treeBypassLog.insert(parent='', index='end', iid=count, text="", values=record,
                                            tags=('oddrow',))
    ...
    def open_tagSetup(self):
        ...
        self.comboBox_tagSetup_triggerTag.bind('<KeyRelease>', self.comboBox_tagSetup_triggerTag_ShortListSort)
        ...
        self.comboBox_tagSetup_databaseKey.bind('<KeyRelease>', self.comboBox_tagSetup_databaseKeyListSort)
        ...
        label6 = customtkinter.CTkLabel(master=self.frame_tagSetup, text="Note: Trigger Tag & Database Key Selection Are Irrelevant If Log Value = No\nCombo Box Values Will Update As You Type", font=customtkinter.CTkFont(size=14,))
        label6.place(x=20,y=960)
    ...
    def open_tagImport(self):
        ...
        # Grab Config Data To Determine if we need to display the omron message
        user, username, plcType, ipAddress = self.connection.configurationRead()[0]

        # Label For Omron PLCs about requirements
        if plcType == 1:
            label5 = customtkinter.CTkLabel(master=self.frame_tagImport, text="Omron Datatypes (UDT) with an Offset Type of User is not supported.\nIn order to properly interface with the PLC program,\nit is necessary to utilize a new Datatype that uses an Offset Type of NJ.\nData Type field will be automatically populated.", font=customtkinter.CTkFont(size=16,), anchor='w')
            label5.place(x=20, y=460, anchor='nw')

       # Update Treeview
        self.updateImportedTagsTreeview()

        # Create Omron Connection in a thread.
        self.ui_thread1 = threading.Thread(target=self.omronConnection, args=(self.event,))
        self.ui_thread1.start()
...
Programming By: Kevin Lay''')
    ...
    # Other Functions
    def comboBox_tagSetup_triggerTag_ShortListSort(self, event):
        self.updateImportTagList()
        current = self.comboBox_tagSetup_triggerTag.get()
        if current == '':
            self.comboBox_tagSetup_triggerTag_ShortList = self.importedTagList
        else:
            data = []
            for item in self.importedTagList:
                if current.lower() in item.lower():
                    data.append(item)
            self.comboBox_tagSetup_triggerTag.configure(values=data)

    def comboBox_tagSetup_databaseKeyListSort(self, event):
        self.updateImportTagList()
        current = self.comboBox_tagSetup_databaseKey.get()
        if current == '':
            self.comboBox_tagSetup_databaseKey = self.importedTagList
        else:
            data = []
            for item in self.importedTagList:
                if current.lower() in item.lower():
                    data.append(item)
            self.comboBox_tagSetup_databaseKey.configure(values=data)
    ...
    def updateImportedTagsTreeview(self):
        try:
            #self.tree_frameTagImport.destroy()
            temp = 0
            temp += 1
        except:
            pass
        ...
        # Setup Scrollable Frame Window
        self.tree_frameTagImport = customtkinter.CTkFrame(master=self.frame_tagImport, border_color="#EB8613",
                                                          border_width=2)
        self.tree_frameTagImport.place(x=1675, y=40, anchor="ne", width=400, height=950)

        # Tag Import Viewing Frame --> Read Value
        label6 = customtkinter.CTkLabel(master=self.tree_frameTagImport, text="List of Imported Tags",
        ...
        # Setup Tree View selectmode="" prevents selecting item
        self.my_treeTagImport = ttk.Treeview(master=self.tree_frameTagImport, height=40,
                                             yscrollcommand=tree_scrollYtagOverview.set)
        ...
        for record in data:
            if count % 2 == 0:  # If we divide count by 2 and the remainder is 0 (even Row)
                self.my_treeTagImport.insert(parent='', index='end', iid=count, text="", values=record,
                                             tags=('evenrow',))
            else:
                self.my_treeTagImport.insert(parent='', index='end', iid=count, text="", values=record,
                                             tags=('oddrow',))
    ...
    def setMainMenuColors(self):
        if self.activeWindow == 2:
            ...
            self.button_mainMenu_bypassLog.configure(fg_color='#1F538D', text_color='white')
        ...
        if self.activeWindow == 5:
            ...
            self.button_mainMenu_tagOverview.configure(fg_color='#1F538D', text_color='white')
            self.button_mainMenu_faultLog.configure(fg_color='#1F538D', text_color='white')
            self.button_mainMenu_bypassLog.configure(fg_color='#EB8613', text_color='black', hover=False)
            self.button_mainMenu_tagSetup.configure(fg_color='#1F538D', text_color='white')
            self.button_mainMenu_tagImport.configure(fg_color='#1F538D', text_color='white')
            self.button_mainMenu_configuration.configure(fg_color='#1F538D', text_color='white')
            self.button_mainMenu_bypassSetup.configure(fg_color='#1F538D', text_color='white')
    ...
    def updateFaultLog(self):
        # Create Imported Tag List
        return self.connection.selectAllFaultLog()

    def updateBypassLog(self):
        # Create Imported Tag List
        return self.connection.selectAllBypassLog()
